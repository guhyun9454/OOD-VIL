    ...
    def evaluate_ood(self, model, id_datasets, ood_dataset, device, args):
        ...
        with torch.no_grad():
            ...
            for inputs, targets in aligned_id_loader:
                ...
                # ICON의 예측 로직을 통해 출력 logits 추출 (class_mask는 모든 클래스)
                ...
                # 임계치 이하이면 unknown label (args.num_classes)
                pred = torch.where(max_softmax < args.ood_threshold, 
                                     torch.full_like(pred_class, args.num_classes), 
                                     pred_class)
            ...
            for inputs, targets in aligned_ood_loader:
                ...
                pred = torch.where(max_softmax < args.ood_threshold, 
                                     torch.full_like(pred_class, args.num_classes), 
                                     pred_class)
                # OOD 데이터의 true label은 unknown (args.num_classes)로 설정
        ...
        if args.verbose:
            from utils import save_confusion_matrix_plot, save_anomaly_histogram
