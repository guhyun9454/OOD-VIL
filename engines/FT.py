...
class Engine:
    ...
    def _dos_enabled(self, args) -> bool:
        """DOS를 --ood_method 로 선택했을 때만 task마다 DOS 파인튜닝을 수행."""
        ood_method = getattr(args, "ood_method", None)
        if not ood_method:
            return False
        ood_method = str(ood_method).upper()
    ...
    def dos_finetune_task(
        ...
        """
        DOS (Diverse Outlier Sampling) fine-tuning (no absent class in this repo):
        - 후보 OOD에서 특징 기반 KMeans 클러스터링
        - 클러스터별 hard 샘플을 선택해 OE(Uniform) 손실에 연결

        loss = CE(ID) + λ * CE(Uniform, softmax(DOS-selected OOD))
        """
        if oe_dataset is None:
        ...
        for ft_epoch in range(int(ft_epochs)):
            ...
            for batch_idx, (x_in, y_in) in enumerate(id_loader):
                ...
                x_sel = dos_select_batch(model, x_can, num_select=num_select, cfg=cfg)

                logits_in = model(x_in)
                loss_id = criterion(logits_in, y_in)

                logits_ood = model(x_sel)
                logp_ood = F.log_softmax(logits_ood, dim=1)
